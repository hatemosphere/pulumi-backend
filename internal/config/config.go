...
type Config struct {
	...
	BackupDestination      string        // backup destination URI: s3://bucket/prefix or gs://bucket/prefix
	BackupS3Region         string        // AWS region (default: us-east-1), S3 only
	BackupS3Endpoint       string        // custom S3 endpoint URL (MinIO, R2, B2), S3 only
	BackupS3ForcePathStyle bool          // force path-style S3 addressing (for MinIO), S3 only
